...
for ij in range(0, 11, +1):
    ...
    if len(line.rstrip()) is not 0:
        san = str(line.rstrip())
        sandisk = san[0:san.find(':')]
        uid, typ  = cutstring(str(line.rstrip()), 'UUID') + ' ' + '/media/' + cutstring(str(line.rstrip()), 'UUID') , cutstring(str(line.rstrip()), 'TYPE')
        dict[ij] = {'sandisk':sandisk, 'UUID':uid, 'TYPE':typ}
for pt in range(0, len(dict), +1):
    if fsStr.find(dict[pt]['UUID'].split(' ')[1]) is -1 and fsStr.find(dict[pt]['UUID'].split(' ')[0]) is -1:
        if dict[pt]['TYPE'] == 'vfat':
            uid, typ, ect = dict[pt]['sandisk'] + ' ' + dict[pt]['UUID'].split(' ')[1], dict[pt]['TYPE'], 'rw,auto,user,fmask=0111,dmask=0000,noatime,nodiratime 0 0'
            Qstring = Qstring + uid + ' ' + typ + ' ' + ect + '\n'
        if dict[pt]['TYPE'] == 'ext4':
            uid, typ, ect = dict[pt]['sandisk'] + ' ' + dict[pt]['UUID'].split(' ')[1], dict[pt]['TYPE'], 'defaults 0 0'
            Qstring = Qstring + uid + ' ' + typ + ' ' + ect + '\n'
os.system('sudo chmod +777 /etc/fstab')
fsStr = fsStr + '\n' + Qstring.replace('"','').replace("b'","")
